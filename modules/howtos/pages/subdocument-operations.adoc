...
include::devguide:example$subdoc-retrieving.cc[]
[source,c]
...
_libcouchbase_ is an asynchronous library which means that operations results are passed to callbacks you define rather than
being returned to functions.
Callbacks are passed `cookie` parameter which is a user defined pointer(i.e your own pointer which can be  NULL) to associate
a specific command with a specific callback invocation.
[source,c]
----
   #define VALUE_SIZE 100876
   static int counter = 0;
   static const char *key = "travel-sample";
...
----
For simple synchronous use, you will need to call `lcb_wait()` after each set of scheduled operations. During `lcb_wait`
the library will block for I/O and invoke your callbacks as the results of the operations arrive.
[source,c]
----
    lcb_cmdstore_create(&cmd, LCB_STORE_UPSERT);
    lcb_cmdstore_key(cmd, key, strlen(key));
