...
(Sub-document {sqlpp} queries are explained in the section xref:concept-docs:n1ql-query.adoc[Querying with {sqlpp}].)
...
[source,c]
----
include::devguide:example$c/subdoc-retrieving.cc[tag=sub-doc-exists]
----

.Retrieve Subdocument value
[source,c]
----
include::devguide:example$c/subdoc-retrieving.cc[tag=sub-doc-retrieve]
----
...
See the {lcb_doc_src_link}/modules/devguide/examples/c/subdoc-retrieving.cc[code sample^] for use in context.
...
See the {lcb_api_link}/example_2minimal_2minimal_8c-example.html#a5[code sample^] for use in context.
...
[source,c]
----
include::devguide:example$c/subdoc-updating.cc[tag=sub-doc-mutate]
----
See the {lcb_doc_src_link}/modules/devguide/examples/c/subdoc-retrieving.cc[code sample^] for use in context.
...
When submitting multiple _mutation_ operations within a single _mutate-in_ command, those operations are considered to be part of a single transaction:
if any of the mutation operations fail, the server will logically roll-back any other mutation operations performed within the _mutate-in_,
