...
We `upsert` an initial version of the document. We don't care about  the exact details of the result,
just whether it succeeded or not, so do not assign a value. Then we get back into  `doc` and pull out the
document's content as a `Json`
