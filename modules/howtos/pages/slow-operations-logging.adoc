...
The metrics logging can be enabled/disabled, and the emit interval can be configured, in the connection string or via `lcb_cntl` calls.  See xref:ref:client-settings.adoc#metrics_reporting-options[metrics reporting options]
...
The `emitInterval` and `sampleSize` can be adjusted, see xref:ref:client-settings.adoc#tracing-orphaned-logging-options[tracing orphaned logging options]
for details.  They default to 10s and 128 samples per service, respectively.
The overall structure looks like this (here prettified for readability): +
