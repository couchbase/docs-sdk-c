...
See the <<metrics_reporting-options, metrics reporting options>>.
...
To configure threshold logging, adjust the <<tracing-threshold-logging-options, tracing threshold logging options>>.
...
The `emitInterval` and `sampleSize` can be adjusted, see the <<tracing-orphaned-logging-options, tracing orphaned logging options>> for details.
