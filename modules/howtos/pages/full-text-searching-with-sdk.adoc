...
[source,c]
----
include::example$fts-basic.cc[tag=query-example,indent=0]
----
== Search Queries
The query string is the simplest form available in the API, but there are many more available.
Please refer to the
https://docs.couchbase.com/sdk-api/couchbase-c-client/group__lcb-cbft-api.html[API Documentation].

== The Search Result
Once a search query is executed sucessfully, the callback will be invoked after each hit.
It will be invoked one last time with the result metadata (including any facets) and the `LCB_RESP_F_FINAL` will return true.

[source,c]
----
include::example$fts-basic.cc[tag=query-result,indent=0]
----
...
//For a full Search example, see the https://docs.couchbase.com/sdk-api/couchbase-c-client/example_2fts_2fts_8c-example.html[API documentation].
