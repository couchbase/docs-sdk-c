...
Before you get started you may wish to checkout the xref:{cb_server_version}@server:n1ql:n1ql-language-reference/index.adoc[N1QL intro page], or just dive in with a query against our travel sample data set.
...
From version 3.0.7 of the C SDK, libcouchbase, it is possible to query off the xref:{cb_server_version}@server:learn:data/scopes-and-collections.adoc[`Scope` level] _with Couchbase Server 7.0_,
...
The xref:{cb_server_version}@server:n1ql:n1ql-language-reference/index.adoc[Server doc N1QL intro] introduces up a complete guide to the N1QL language, including all of the latest additions.
