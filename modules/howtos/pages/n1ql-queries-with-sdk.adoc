...
[source,c]
----
include::example$query-criteria.cc[tag=query,indent=0]
----
...
[source,c]
----
include::example$query-placeholders.cc[tag=placeholder,indent=0]
----
...
By default the results are read from a potentially stale snapshot of data. This may be good for most cases; however at times you want the absolutely most recent data.
If you need to ensure that mutations up to the vector indicated by the mutation token passed to `lcb_cmdquery_consistency_token_for_keyspace` are used , use `LCB_QUERY_CONSISTENCY_RYROW`.
`LCB_QUERY_CONSISTENCY_REQUEST` refreshes the snapshot for each request, while `LCB_QUERY_CONSISTENCY_STATEMENT` refreshes the snapshot for each statement.

[source,c]
----
include::example$query-consistency.cc[tag=consistency,indent=0]
----

== Async APIs

For full details on Async API, please visit
https://docs.couchbase.com/c-sdk/current/howtos/concurrent-async-apis.html[Async and Batching APIs].
...
From version 3.0.7 of the C SDK, libcouchbase, it is possible to query off the xref:7.0@server:learn:data/scopes-and-collections.adoc[`Scope` level] _with the beta version of the next Couchbase Server release, 7.0β_,
...
As well as the https://docs.couchbase.com/sdk-api/couchbase-c-client/group__lcb-n1ql-api.html[API docs], there are examples in the GitHub repo for:

* https://github.com/couchbase/docs-sdk-c/blob/release/3.1/modules/devguide/examples/c/query-criteria.cc[Query Example]
* https://github.com/couchbase/docs-sdk-c/blob/release/3.1/modules/devguide/examples/c/query-placeholders.cc[Querying with Placeholders]
* https://github.com/couchbase/docs-sdk-c/blob/release/3.1/modules/devguide/examples/c/query-consistency.cc[Scan Consistency]
* https://github.com/couchbase/docs-sdk-c/blob/release/3.1/modules/devguide/examples/c/query-atplus.cc[AtPlus (Read Your Own Writes)]


== Querying at Scope Level
[.status]#Developer Preview#

From version 3.0.4 of the C SDK, it is possible to query off the xref:7.0@server:learn:data/scopes-and-collections.adoc[`Scope` level] _with the beta version of the next Couchbase Server release, 7.0β_,
...
usage details for this and `lcb_cmdquery_scope_qualifier()` can be found in the https://docs.couchbase.com/sdk-api/couchbase-c-client/group__lcb-n1ql-api.html#gadcd89153027afd789e2d31e250424c48[API docs].
