...
The orphan logger can be configured through the the connection string, or using `lcb_cntl` calls.  See xref:ref:client-settings.adoc#tracing-orphaned-logging-options[tracing orphaned logging options] for details.
