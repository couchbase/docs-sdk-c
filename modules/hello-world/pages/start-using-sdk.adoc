...
[abstract]
{description}
It was used by older versions of the Node.js, PHP, and Python SDKs to communicate with the Couchbase Server.
...
Couchbase uses xref:{version-common}@server:learn:security/roles.adoc[Role Based Access Control (RBAC)] to control access to resources.
...
A named or default xref:{version-common}@server:learn:data/scopes-and-collections.adoc[collection] will provide the same functionality as bucket-level operations did in previous versions of Couchbase Server.
