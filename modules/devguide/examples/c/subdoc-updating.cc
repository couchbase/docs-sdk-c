...
main() {
...

    {
        ...
        check(lcb_subdocspecs_array_add_last(specs, 0, 0, paths[0].data(), paths[0].size(),
                                             value_to_add.data(),
                                             value_to_add.size()),
              "create ARRAY_ADD_LAST operation");
        // end::array-insert[]
        check(lcb_subdocspecs_counter(specs, 1, 0, paths[1].data(), paths[1].size(), 99),
              "create COUNTER operation");

        std::string value_to_upsert{R"("just a dev")"};
        check(lcb_subdocspecs_dict_upsert(specs, 2, 0, paths[2].data(), paths[2].size(),
                                          value_to_upsert.data(),
    ...
    // Show how to set command options!
    {
        ...
        check(lcb_subdocspecs_dict_upsert(specs, 0, 0, path.data(), path.size(), value.data(),
                                          value.size()),
