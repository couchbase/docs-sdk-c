...
main()
{
    ...
    lcb_install_callback(instance, LCB_CALLBACK_STORE, reinterpret_cast<lcb_RESPCALLBACK>(upsert_callback));
    ...
    for (const auto& doc : documents_to_store) {
        ...
        check(lcb_cmdstore_key(cmd, doc.first.c_str(), doc.first.size()), "assign ID for UPSERT command");
        check(lcb_cmdstore_value(cmd, doc.second.c_str(), doc.second.size()), "assign value for UPSERT command");
        lcb_STATUS rc = lcb_store(instance, &results, cmd);
        check(lcb_cmdstore_destroy(cmd), "destroy UPSERT command");
        if (rc != LCB_SUCCESS) {
            std::cerr << "[ERROR] could not schedule UPSERT for " << doc.first << ": " << lcb_strerror_short(rc) << "\n";
