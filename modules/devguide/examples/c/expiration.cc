...
static void
store_key(lcb_INSTANCE *instance, const std::string &key, const std::string &value,
    ...
    check(lcb_cmdstore_expiry(cmd, static_cast<std::uint32_t>(expiry.count())),
          "assign expiration to UPSERT command");
...
get_key(lcb_INSTANCE *instance, const std::string &key, std::chrono::seconds expiry = {}) {
    ...
    check(lcb_cmdget_expiry(cmd, static_cast<std::uint32_t>(expiry.count())),
          "assign expiration to GET command");
...
touch_key(lcb_INSTANCE *instance, const std::string &key, std::chrono::seconds expiry) {
    ...
    check(lcb_cmdtouch_key(cmd, key.data(), key.size()), "assign ID for TOUCH command");
    check(lcb_cmdtouch_expiry(cmd, static_cast<std::uint32_t>(expiry.count())),
          "assign expiration to TOUCH command");
...
main(int, char **) {
    ...
    lcb_install_callback(instance, LCB_CALLBACK_TOUCH,
                         reinterpret_cast<lcb_RESPCALLBACK>(touch_callback));
