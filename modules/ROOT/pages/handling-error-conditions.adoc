...
Note that any error code other than [.api]`LCB_HTTP_ERROR` indicates that either the C SDK has handled and converted HTTP or metadata-reported errors, or that an error occurred at the transport or input validation layer.
