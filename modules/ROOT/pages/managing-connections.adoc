...
IPv6 can be specified as allowed or preferred through the connection string `ipv6=allow`, or directly with the C SDK, since version xref:sdk-release-notes.adoc#2-8-3-november-22-2017[2.8.3] of LCB -- see the xref:client-settings.adoc#settings-list[Client Settings] page.

The option can also be passed in the Connection String:

[source,c]
----
couchbases://host1.example.com?ipv6=allow
----
...
Note, `config_cache` -- implemented with `LCB_CNTL_CONFIGCACHE` or `lcb_cntl_string()` -- uses `connstr` internally in either case, which is set _per bucket_.
// end::configcache[]
