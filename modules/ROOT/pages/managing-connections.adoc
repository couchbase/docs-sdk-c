...
include::4.6@sdk:shared:partial$lcb-pars-shared.adoc[tag=connstr-intro]
...
If the bucket is password protected, the [.param]`passwd` field may be set.
Note that the bucket password (set when creating a bucket) is not the same as the administrative password for Couchbase.
