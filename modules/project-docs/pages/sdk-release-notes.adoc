...
Enable out of order execution by default

* https://issues.couchbase.com/browse/CCBC-1189[CCBC-1189]:
Validate collection and scope names early. The library will return error if the collection name is not valid without
sending command to the server.

* https://issues.couchbase.com/browse/CCBC-1278[CCBC-1278]:
Fix stripping payload over TLS connection. The library might propagate zero return code from `recv()` even when previous
calls of `recv()` successfully transferred bytes to SSL context. In this case we might lose these trailing bytes and just
close the socket.

* https://issues.couchbase.com/browse/CCBC-1264[CCBC-1264]:
Fix calculating key size when durability level specified

* https://issues.couchbase.com/browse/CCBC-1258[CCBC-1258]:
Fix port order in vbucket diff info message

* https://issues.couchbase.com/browse/CCBC-1274[CCBC-1274]:
Fix pointer casting in HTTP callback of cbc tool.
...
Always request error map feature and send HELLO message to speedup bootstrap
...
| Ubuntu 18.04 (bionic)         | x64 | https://packages.couchbase.com/clients/c/libcouchbase-3.0.3_ubuntu2004_focal_amd64.tar[libcouchbase-3.0.3_ubuntu2004_focal_amd64.tar]
