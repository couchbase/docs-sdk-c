...
* https://issues.couchbase.com/browse/CCBC-1448[CCBC-1448]:
  Fixed cmake macro for examples. The macro `ADD_EXAMPLE` now correctly updates header directories for generated target.

* https://issues.couchbase.com/browse/CCBC-1445[CCBC-1445]:
  Always invoke callbacks for pending HTTP operations. When destroying `lcb_INSTANCE` make sure that all HTTP requests (views, query, search, analytics) will have their callbacks invoked.
...
| Debian 11 (bullseye)          | x64 | https://packages.couchbase.com/clients/c/libcouchbase-3.2.0_debian11_bullseye_amd64.tar[libcouchbase-3.2.0_debian11_bullseye_amd64.tar]
