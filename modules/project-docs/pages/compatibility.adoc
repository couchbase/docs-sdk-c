...
① The C SDK 3.3.12 is compatible with Server 7.6, but does not contain APIs for some of the 
xref:7.6@server:introduction:whats-new.adoc[newer features],
specifically LCB 3.3.x does not contain APIs for Vector Search, Scoped Search Indexes, KV Range Scan, or Sub-Doc Read-from-Replica.
...
Note, LCB does not bundle the Capella Client Certificate, follow the xref:server:guides:connect.adoc#tls[instructions] to download and install the certificate from Capella, then the guide to xref:howtos:managing-connections.adoc#ssl[secure connections with LCB].
...
* Debian 10 (_Buster_), Debian 11 (_Bullseye_), and (since LCB 3.13) Debian 12 (_Bookworm_).
* Alpine Linux (Since LCB 3.2.5)

NOTE: Some of the OS versions listed above were supported at time of LCB 3.3 release, but have since gone End of Life, so are no longer supported.
See the xref:sdk-release-notes.adoc#latest-release[release notes page] for the platforms built with the latest release.
...
[cols="7,5,6,5"]
|===
| | Server 6.0 | Server 6.5 & 6.6 | Server 7.0 - 7.6
...
include::7.5@sdk:shared:partial$api-version.adoc[tag=api-version]
